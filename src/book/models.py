...
class Genre(db.Model, CreatedUpdatedMixin, CRUDMixin):
    ...
    name = db.Column(
        db.String(length=50), nullable=False, doc="Genre name", primary_key=True
    )
...
class Book(db.Model, IdMixin, CreatedUpdatedMixin, CRUDMixin):
    ...
    doi = db.Column(db.String(length=50), doc="DOI", unique=True)
    publisher = db.Column(db.String(length=255), doc="Publisher")
    url = db.Column(db.Text, doc="Source URL")
    source = db.Column(db.String(length=255), doc="Source name")
    ...
    price = db.Column(db.Float, default=0.0, doc="Price")
